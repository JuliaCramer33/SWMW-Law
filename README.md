...
- **Staging:** Push to `develop` → builds theme + plugin, deploys to WP Engine staging (`wpe_env` in `deploy-staging.yml`).
- **Production:** Push to `main` → same build, deploys to WP Engine production (`wpe_env` in `deploy-production.yml`).
...
### Handoff: putting this repo on your own GitHub

If you're receiving this codebase to host on **your** GitHub and run deploys:

1. **Create a new repo** (empty, no README) under your org/account.
2. **Get the code** into it: either have the sender push to your repo (as a collaborator), or have them send you a `git bundle` and then `git clone swmw-law.bundle <dir> && cd <dir> && git remote add origin <your-repo-url> && git push -u origin main && git push origin develop`.
3. **Add the deploy secret** (see **Setup** above): `WPE_SSHG_KEY_PRIVATE` in **Settings → Secrets and variables → Actions**.
4. **Set your WP Engine env names** in `deploy-staging.yml` and `deploy-production.yml` (the `wpe_env` value in each).

After that, pushes to `develop` deploy to staging and pushes to `main` deploy to production.
...
wp-content/
├── .github/workflows/
│   ├── build-and-deploy.yml  # reusable build + deploy
│   ├── deploy-staging.yml    # develop → staging (wpe_env)
│   └── deploy-production.yml # main → production (wpe_env)
├── .deployignore
├── plugins/
...
This should be enough for an outside developer to understand the stack, what’s built, and how to spin up and deploy the site.
