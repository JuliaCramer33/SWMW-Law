...
abstract class Task_Runner {
	...
	public static function run_tasks( $process_key = '' ) {
		...
		// CRITICAL FIX: Only set 'finished' if we're still the current generation.
		// This prevents a stale process from overriding a new rebuild's status.
		if ( self::has_finished_tasks() && self::is_current_generation( $process_generation ) ) {
	...
	private static function set_error_count( $error_count = 0 ) {
		$option_name = self::$option_error_name . static::$type;
		Options::update( $option_name, $error_count );
	}
	...
	public static function create_process_key() {
		...
		try {
			Options::create( $option_name, $process_key );
		} catch ( \Exception $e ) {
			$process_key = false;
		}
		return $process_key;
