...
class Query {
	...
	public function __construct( $query ) {
		...
		if ( ! empty( $all_filter_field_bitmaps ) ) {
			$combined_bitmap = $this->combine_bitmaps( $all_filter_field_bitmaps, $combine_type );
		...
		if ( ! empty( $fields_by_strategy['search'] ) ) {
			...
			if ( $search_failed_query ) {
				$post__in = array(); // Signify no results found.
			} else {
				// Combine search field results.
				$combined_search_bitmap = $this->combine_bitmaps( $all_search_field_bitmaps, $combine_type );
		...
		foreach ( $fields_without_strategy as $field ) {
	 ...
	 */
	private function combine_bitmaps( $bitmaps, $combine_type = 'intersect' ) {
	...
	public function get_combined_result_field_bitmaps_excluding( $exclude_field_id ) {
		...
		return $this->combine_bitmaps( $all_field_result_bitmaps, $combine_type );
	}
