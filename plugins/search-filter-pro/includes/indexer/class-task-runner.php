...
class Task_Runner extends Base_Task_Runner {
	...
	protected static function run_task( &$task ) {
		...
		switch ( $task->get_action() ) {
			...
			case 'remove_field':
				...
				break;
			case 'rebuild_bucket':
				self::task_rebuild_bucket( $task );
				break;
			case 'migrate':
				self::task_migrate( $task );
				break;
			case 'sync_post':
				self::task_post_sync( $task );
				break;
			case 'sync_post_batch':
				self::task_post_batch_sync( $task );
				break;
			default:
	 ...
	 * @param Task $task The task to process.
	 */
	private static function task_rebuild_bucket( $task ) {
		$field_id = $task->get_meta( 'field_id', true );
	 ...
	 * @param Task $task The task to process.
	 */
	private static function task_migrate( &$task ) {
	...
	private static function generate_rebuild_index_tasks( &$task, $post_types, $additional_task_data = array() ) {
		...
		$query_result  = self::get_posts_for_indexing( $post_types, $page_number );
	 ...
	 * @param Task $task The task to process.
	 */
	private static function task_post_batch_sync( $task ) {
		...
		// Phase 1: Collect values for all posts.
		foreach ( $post_ids as $post_id ) {
			$post = get_post( $post_id );
			if ( ! $post ) {
				continue;
