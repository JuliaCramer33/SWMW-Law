...
class Query_Builder {
	...
	private function search_exact( $query, array $options = array() ) {
		...
		$profile_start = defined( 'SF_PROFILE_SEARCH' ) && SF_PROFILE_SEARCH ? hrtime( true ) : null;
		...
		$profile_tokenize_start = $profile_start ? hrtime( true ) : null;
		$tokenizer              = $this->get_tokenizer();
		...
		if ( $profile_tokenize_start ) {
			do_action( 'search_filter_pro/search/profile/tokenize', ( hrtime( true ) - $profile_tokenize_start ) / 1e6, count( $query_terms ) );
		}
		...
		$profile_terms_start = $profile_start ? hrtime( true ) : null;
		$term_data           = Search_Query_Direct::get_terms_data( $query_terms, $options['language'] );
		if ( $profile_terms_start ) {
			do_action( 'search_filter_pro/search/profile/term_lookup', ( hrtime( true ) - $profile_terms_start ) / 1e6, count( $term_data ) );
		}
		...
		$profile_postings_start = $profile_start ? hrtime( true ) : null;
		$postings               = Search_Query_Direct::get_postings_for_terms(
			array_keys( $term_data ),
			$options['allowed_object_ids'],
			$options['field_id']
		);
		if ( $profile_postings_start ) {
			do_action( 'search_filter_pro/search/profile/postings_fetch', ( hrtime( true ) - $profile_postings_start ) / 1e6, count( $postings ), $options['allowed_object_ids'] ? count( $options['allowed_object_ids'] ) : null );
		}
		...
		if ( $use_two_pass ) {
			...
			$profile_two_pass_start      = $profile_start ? hrtime( true ) : null;
			...
			if ( $profile_two_pass_start ) {
				do_action( 'search_filter_pro/search/profile/two_pass', ( hrtime( true ) - $profile_two_pass_start ) / 1e6, count( $object_ids ) );
			}
		} else {
			// Normal result set: Use full BM25 (default).
			$profile_bm25_start          = $profile_start ? hrtime( true ) : null;
			...
			if ( $profile_bm25_start ) {
				do_action( 'search_filter_pro/search/profile/bm25_calc', ( hrtime( true ) - $profile_bm25_start ) / 1e6, count( $object_ids ) );
			}

			// Profile: Sorting.
			$profile_sort_start = $profile_start ? hrtime( true ) : null;
			array_multisort( $scores, SORT_DESC, $object_ids );
			if ( $profile_sort_start ) {
				do_action( 'search_filter_pro/search/profile/sort', ( hrtime( true ) - $profile_sort_start ) / 1e6, count( $object_ids ) );
			}
		}
		...
		if ( 'bitmap' === $options['return_format'] ) {
			$profile_bitmap_start = $profile_start ? hrtime( true ) : null;
			$result               = Bitmap::from_post_ids( $object_ids );
			if ( $profile_bitmap_start ) {
				do_action( 'search_filter_pro/search/profile/bitmap_convert', ( hrtime( true ) - $profile_bitmap_start ) / 1e6, count( $object_ids ) );
			}
			if ( $profile_start ) {
				do_action( 'search_filter_pro/search/profile/total', ( hrtime( true ) - $profile_start ) / 1e6 );
			}
			return $result;
		}
		...
		return $object_ids;
	// phpcs:enable PHPCompatibility.FunctionUse.NewFunctions.hrtimeFound
	}
