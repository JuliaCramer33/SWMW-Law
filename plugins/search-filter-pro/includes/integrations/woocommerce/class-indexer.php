...
class Indexer {
	...
	public static function index_product_values( $values, $field, $object_id ) {
		...
		// For variable products that don't have variations yet, we need to store on the
		// parent product.
		if ( $product->is_type( 'variable' ) && ! empty( $product->get_children() ) ) {
			// Return empty array to explicitly skip default extraction.
			return array();
		}
	 ...
	 */
	private static function extract_with_inheritance( $variation, $parent, $field ) {
		$behavior = self::get_inheritance_behavior( $field );

		switch ( $behavior ) {
			case 'inherit':
				// Use parent only.
				return self::get_index_values( $parent, $field );

			case 'combine':
				// Merge variation + parent values.
				$variation_values = self::get_index_values( $variation, $field );
				$parent_values    = self::get_index_values( $parent, $field );
			...
			case 'inherit_or_own':
				...
				if ( empty( $result ) ) {
					$result = self::get_index_values( $parent, $field );
				}
				return $result;

			case 'smart':
				// Check WC attribute metadata.
				return self::extract_smart_attribute( $variation, $parent, $field );
	...
	private static function extract_woocommerce_values( $product, $field ) {
		...
		if ( $wc_data === 'stock_status' ) {
			...
			// TODO - watch out for the release of a new function `get_brand_ids()` which does not
			// exist yet, but likely will later: https://github.com/woocommerce/woocommerce/issues/52991.
	 ...
	 * @return array Array of index values.
	 */
	private static function extract_smart_attribute( $variation, $parent, $field ) {
		$wc_data = $field->get_attribute( 'dataWoocommerce' );
