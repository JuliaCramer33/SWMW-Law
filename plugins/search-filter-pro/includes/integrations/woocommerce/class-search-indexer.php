...
class Search_Indexer {
	...
	public static function index_variation_search_content( $content, $source, $field, $object_id ) {
		...
		$values = self::extract_with_inheritance( $variation, $parent, $source );
	 ...
	 */
	private static function extract_with_inheritance( $variation, $parent, $source ) {
		$behavior = self::get_inheritance_behavior( $source );

		switch ( $behavior ) {
			case 'inherit':
				// Use parent only.
				return self::get_search_content( $parent, $source );

			case 'combine':
				// Merge variation + parent values.
				$variation_values = self::get_search_content( $variation, $source );
				$parent_values    = self::get_search_content( $parent, $source );
			...
			case 'inherit_or_own':
				...
				if ( empty( $values ) ) {
					$values = self::get_search_content( $parent, $source );
				}
				return $values;

			case 'smart':
				// Check WC attribute metadata.
				return self::extract_smart_attribute( $variation, $parent, $source );
	 ...
	 * @return array Array of searchable values.
	 */
	private static function extract_smart_attribute( $variation, $parent, $source ) {
		$wc_data = $source['dataWoocommerce'] ?? '';
		...
		if ( $product_attribute->get_variation() ) {
			...
			$values = array();
			$terms  = wp_get_post_terms( $parent->get_id(), $taxonomy_name );
