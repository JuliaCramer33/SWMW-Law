...
final class ASCII {
	 ...
	 * @var string
	 */
	private static $REGEX_ASCII = "[^\t\x10\x13\n\r -~]";
	/**
	...
	public static function charsArray( bool $replace_extra_symbols = \false ): array {
		if ( $replace_extra_symbols ) {
			...
			return self::$ASCII_MAPS_AND_EXTRAS ?? array();
		}
		self::prepareAsciiMaps();
		return self::$ASCII_MAPS ?? array();
	}
	/**
	...
	public static function charsArrayWithMultiLanguageValues( bool $replace_extra_symbols = \false ): array {
		...
		// init
		$return             = array();
		$language_all_chars = self::charsArrayWithSingleLanguageValues( $replace_extra_symbols, \false );
	 ...
	 */
	public static function charsArrayWithOneLanguage( string $language = self::ENGLISH_LANGUAGE_CODE, bool $replace_extra_symbols = \false, bool $asOrigReplaceArray = \true ): array {
		...
		return $CHARS_ARRAY[ $cacheKey ][ $language ] ?? array(
			'orig'    => array(),
			'replace' => array(),
		);
	}
	 ...
	 * @return ($asOrigReplaceArray is true ? array{orig: list<string>, replace: list<string>} : array<string, string>)
	 */
	public static function charsArrayWithSingleLanguageValues( bool $replace_extra_symbols = \false, bool $asOrigReplaceArray = \true ): array {
		// init
		static $CHARS_ARRAY = array();
		$cacheKey           = '' . $replace_extra_symbols . '-' . $asOrigReplaceArray;
		if ( isset( $CHARS_ARRAY[ $cacheKey ] ) ) {
		...
		if ( $replace_extra_symbols ) {
			self::prepareAsciiAndExtrasMaps();
			/* @noinspection AlterInForeachInspection | ok here */
			foreach ( self::$ASCII_MAPS_AND_EXTRAS ?? array() as &$map ) {
				$CHARS_ARRAY[ $cacheKey ][] = $map;
			}
		} else {
			self::prepareAsciiMaps();
			/* @noinspection AlterInForeachInspection | ok here */
			foreach ( self::$ASCII_MAPS ?? array() as &$map ) {
				$CHARS_ARRAY[ $cacheKey ][] = $map;
			}
		}
		$CHARS_ARRAY[ $cacheKey ] = \array_merge( array(), ...$CHARS_ARRAY[ $cacheKey ] );
		if ( $asOrigReplaceArray ) {
	 ...
	 */
	public static function clean( string $str, bool $normalize_whitespace = \true, bool $keep_non_breaking_space = \false, bool $normalize_msword = \true, bool $remove_invisible_characters = \true ): string {
	...
	public static function normalize_msword( string $str ): string {
		...
		if ( empty( $MSWORD_CACHE['orig'] ) ) {
			self::prepareAsciiMaps();
			$map          = self::$ASCII_MAPS[ self::EXTRA_MSWORD_CHARS_LANGUAGE_CODE ] ?? array();
			$MSWORD_CACHE = array(
				'orig'    => \array_keys( $map ),
	 ...
	 */
	public static function normalize_whitespace( string $str, bool $keepNonBreakingSpace = \false, bool $keepBidiUnicodeControls = \false, bool $normalize_control_characters = \false ): string {
		if ( $str === '' ) {
			return '';
		}
		...
		if ( ! isset( $WHITESPACE_CACHE[ $cacheKey ] ) ) {
			self::prepareAsciiMaps();
			$WHITESPACE_CACHE[ $cacheKey ] = self::$ASCII_MAPS[ self::EXTRA_WHITESPACE_CHARS_LANGUAGE_CODE ] ?? array();
			if ( $keepNonBreakingSpace ) {
				unset( $WHITESPACE_CACHE[ $cacheKey ][' '] );
			}
	...
	public static function remove_invisible_characters( string $str, bool $url_encoded = \false, string $replacement = '', bool $keep_basic_control_characters = \true ): string {
		...
		if ( $keep_basic_control_characters ) {
			...
			$str                = self::normalize_whitespace( $str, \false, \false, \true );
	 ...
	 * @phpstan-param ASCII::*_LANGUAGE_CODE $language
	 */
	public static function to_ascii( string $str, string $language = self::ENGLISH_LANGUAGE_CODE, bool $remove_unsupported_chars = \true, bool $replace_extra_symbols = \false, bool $use_transliterate = \false, bool $replace_single_chars_only = \false ): string {
		if ( $str === '' ) {
			return '';
		}
		...
		static $REPLACE_HELPER_CACHE = array();
		...
		if ( ! isset( $REPLACE_HELPER_CACHE[ $cacheKey ] ) ) {
			...
			if ( $langSpecific === array() ) {
				$REPLACE_HELPER_CACHE[ $cacheKey ] = $langAll;
			} else {
				$REPLACE_HELPER_CACHE[ $cacheKey ] = \array_merge( array(), $langAll, $langSpecific );
			}
		}
		if ( $replace_extra_symbols && $EXTRA_SYMBOLS_CACHE === null ) {
			$EXTRA_SYMBOLS_CACHE = array();
			foreach ( self::$ASCII_EXTRAS ?? array() as $extrasDataTmp ) {
		...
		if ( \preg_match_all( '/' . self::$REGEX_ASCII . ( $replace_extra_symbols ? '|[' . $EXTRA_SYMBOLS_CACHE . ']' : '' ) . '/u', $str, $matches ) ) {
			if ( ! $replace_single_chars_only ) {
				...
				if ( $maxKeyLength >= 5 ) {
					foreach ( $matches[0] as $keyTmp => $char ) {
						...
						if ( $fiveChars && ! isset( $charDone[ $fiveChars ] ) && isset( $REPLACE_HELPER_CACHE[ $cacheKey ][ $fiveChars ] ) && \strpos( $str, $fiveChars ) !== \false ) {
							// DEBUG
							// \var_dump($str, $fiveChars, $REPLACE_HELPER_CACHE[$cacheKey][$fiveChars]);
							$charDone[ $fiveChars ] = \true;
				...
				if ( $maxKeyLength >= 4 ) {
					foreach ( $matches[0] as $keyTmp => $char ) {
						...
						if ( $fourChars && ! isset( $charDone[ $fourChars ] ) && isset( $REPLACE_HELPER_CACHE[ $cacheKey ][ $fourChars ] ) && \strpos( $str, $fourChars ) !== \false ) {
							// DEBUG
							// \var_dump($str, $fourChars, $REPLACE_HELPER_CACHE[$cacheKey][$fourChars]);
							$charDone[ $fourChars ] = \true;
				...
				foreach ( $matches[0] as $keyTmp => $char ) {
					...
					if ( $threeChars && ! isset( $charDone[ $threeChars ] ) && isset( $REPLACE_HELPER_CACHE[ $cacheKey ][ $threeChars ] ) && \strpos( $str, $threeChars ) !== \false ) {
						// DEBUG
						// \var_dump($str, $threeChars, $REPLACE_HELPER_CACHE[$cacheKey][$threeChars]);
						$charDone[ $threeChars ] = \true;
				...
				foreach ( $matches[0] as $keyTmp => $char ) {
					...
					if ( $twoChars && ! isset( $charDone[ $twoChars ] ) && isset( $REPLACE_HELPER_CACHE[ $cacheKey ][ $twoChars ] ) && \strpos( $str, $twoChars ) !== \false ) {
						// DEBUG
						// \var_dump($str, $twoChars, $REPLACE_HELPER_CACHE[$cacheKey][$twoChars]);
						$charDone[ $twoChars ] = \true;
			...
			foreach ( $matches[0] as $char ) {
				if ( ! isset( $charDone[ $char ] ) && isset( $REPLACE_HELPER_CACHE[ $cacheKey ][ $char ] ) && \strpos( $str, $char ) !== \false ) {
					// DEBUG
	 ...
	 */
	public static function to_filename( string $str, bool $use_transliterate = \true, string $fallback_char = '-' ): string {
		if ( $use_transliterate ) {
		...
		$str                   = (string) \preg_replace(
			array(
				...
				'/\s+/u',
				// 2) convert spaces to $fallback_char
				'/[' . $fallback_char_escaped . ']+/u',
			),
			array( '', $fallback_char, $fallback_char ),
			$str
		);
	 ...
	 * @phpstan-param ASCII::*_LANGUAGE_CODE $language
	 */
	public static function to_slugify( string $str, string $separator = '-', string $language = self::ENGLISH_LANGUAGE_CODE, array $replacements = array(), bool $replace_extra_symbols = \false, bool $use_str_to_lower = \true, bool $use_transliterate = \false ): string {
		if ( $str === '' ) {
			return '';
		}
		...
		$str = self::to_ascii( $str, $language, \false, $replace_extra_symbols, $use_transliterate );
		$str = \str_replace( '@', $separator, $str );
		$str = (string) \preg_replace( '/[^a-zA-Z\d\s\-_' . \preg_quote( $separator, '/' ) . ']/', '', $str );
		if ( $use_str_to_lower ) {
			$str = \strtolower( $str );
		}
	...
	public static function to_transliterate( string $str, $unknown = '?', bool $strict = \false ): string {
		...
		if ( $str_tmp !== $str && self::is_ascii( $str ) ) {
			return $str;
		}
		if ( $strict && $SUPPORT_INTL === \true ) {
			...
			if ( $str_tmp !== \false ) {
				// check again if we only have ASCII, now ...
				if ( $str_tmp !== $str && self::is_ascii( $str_tmp ) ) {
					return $str_tmp;
				}
				$str = $str_tmp;
			}
		...
		foreach ( $chars as &$c ) {
			...
			if ( $ordC0 === 254 || $ordC0 === 255 || $ord === null ) {
				$str_tmp .= $unknown ?? $c;
				continue;
			}
			$bank = $ord >> 8;
	...
	private static function prepareAsciiAndExtrasMaps() {
		if ( self::$ASCII_MAPS_AND_EXTRAS === null ) {
			...
			self::$ASCII_MAPS_AND_EXTRAS = \array_merge_recursive( self::$ASCII_MAPS ?? array(), self::$ASCII_EXTRAS ?? array() );
		}
	}
	/**
	 * @return void
