...
namespace Search_Filter_Pro\Vendor\voku\helper;
...
final class UTF8 {
	...
	private static $COMMON_CASE_FOLD = array(
		'upper' => array( 'µ', 'ſ', 'ͅ', 'ς', 'ẞ', 'ϐ', 'ϑ', 'ϕ', 'ϖ', 'ϰ', 'ϱ', 'ϵ', 'ẛ', 'ι' ),
		'lower' => array( 'μ', 's', 'ι', 'σ', 'ß', 'β', 'θ', 'φ', 'π', 'κ', 'ρ', 'ε', 'ṡ', 'ι' ),
	);
	/**
	 * @var array
	 *
	...
	public static function array_change_key_case( array $array, int $case = \CASE_LOWER, string $encoding = 'UTF-8' ): array {
		...
		foreach ( $array as $key => &$value ) {
			$key            = $case === \CASE_LOWER ? self::strtolower( $key, $encoding ) : self::strtoupper( $key, $encoding );
			$return[ $key ] = $value;
		}
		return $return;
	}
	...
	public static function between( string $str, string $start, string $end, int $offset = 0, string $encoding = 'UTF-8' ): string {
		if ( $encoding === 'UTF-8' ) {
			...
			if ( $end_position === \false || $end_position === $substr_index ) {
				return '';
			}
		...
		return (string) self::substr( $str, $substr_index, $end_position - $substr_index, $encoding );
	}
	/**
	...
	public static function checkForSupport() {
		if ( ! isset( self::$SUPPORT['already_checked_via_portable_utf8'] ) ) {
			self::$SUPPORT['already_checked_via_portable_utf8'] = \true;
			// http://php.net/manual/en/book.mbstring.php
			self::$SUPPORT['mbstring']               = self::mbstring_loaded();
			self::$SUPPORT['mbstring_func_overload'] = self::mbstring_overloaded();
			if ( self::$SUPPORT['mbstring'] === \true ) {
			...
			if ( self::$SUPPORT['symfony_polyfill_used'] === \true ) {
				\mb_internal_encoding( 'UTF-8' );
				self::$SUPPORT['mbstring_internal_encoding'] = 'UTF-8';
			}
			return \true;
	...
	public static function chr( $code_point, string $encoding = 'UTF-8' ) {
		...
		if ( $encoding !== 'UTF-8' && $encoding !== 'ISO-8859-1' && $encoding !== 'WINDOWS-1252' && self::$SUPPORT['mbstring'] === \false ) {
			/**
		...
		if ( $code_point <= 0x7ff ) {
			 ...
			 * @psalm-suppress PossiblyNullArrayAccess
			 */
			$chr = self::$CHR[ ( $code_point >> 6 ) + 0xc0 ] . self::$CHR[ ( $code_point & 0x3f ) + 0x80 ];
		} elseif ( $code_point <= 0xffff ) {
			/**
			 * @psalm-suppress PossiblyNullArrayAccess
			 */
			$chr = self::$CHR[ ( $code_point >> 12 ) + 0xe0 ] . self::$CHR[ ( $code_point >> 6 & 0x3f ) + 0x80 ] . self::$CHR[ ( $code_point & 0x3f ) + 0x80 ];
		} else {
			/**
			 * @psalm-suppress PossiblyNullArrayAccess
			 */
			$chr = self::$CHR[ ( $code_point >> 18 ) + 0xf0 ] . self::$CHR[ ( $code_point >> 12 & 0x3f ) + 0x80 ] . self::$CHR[ ( $code_point >> 6 & 0x3f ) + 0x80 ] . self::$CHR[ ( $code_point & 0x3f ) + 0x80 ];
		}
		if ( $encoding !== 'UTF-8' ) {
			$chr = self::encode( $encoding, $chr );
		}
	...
	public static function chr_map( $callback, string $str ): array {
		return \array_map( $callback, self::str_split( $str ) );
	}
	/**
	 ...
	 */
	public static function clean( string $str, bool $remove_bom = \false, bool $normalize_whitespace = \false, bool $normalize_msword = \false, bool $keep_non_breaking_space = \false, bool $replace_diamond_question_mark = \false, bool $remove_invisible_characters = \true, bool $remove_invisible_characters_url_encoded = \false ): string {
	...
	public static function cleanup( $str ): string {
		...
		return self::clean( $str, \true, \true, \false, \true, \true );
	}
	/**
	...
	public static function codepoints( $arg, bool $use_u_style = \false ): array {
		...
		$arg = \array_map( array( self::class, 'ord' ), $arg );
		if ( $use_u_style ) {
			$arg = \array_map( array( self::class, 'int_to_hex' ), $arg );
		}
	 ...
	 */
	public static function count_chars( string $str, bool $clean_utf8 = \false, bool $try_to_use_mb_functions = \true ): array {
		return \array_count_values( self::str_split( $str, 1, $clean_utf8, $try_to_use_mb_functions ) );
	}
	/**
	 ...
	 */
	public static function css_identifier( string $str = '', array $filter = array(
		' ' => '-',
		'/' => '-',
		'[' => '',
		']' => '',
	), bool $strip_tags = \false, bool $strtolower = \true ): string {
		...
		// fallback (1)
		if ( ! $str ) {
			$str = \uniqid( 'auto-generated-css-class', \true );
		}
		if ( $strtolower ) {
			$str = \strtolower( $str );
		}
		...
		$str = (string) \preg_replace( array( '/^[0-9]/', '/^(-[0-9])|^(--)/' ), array( '_', '__' ), $str );
		return \trim( $str, '-' );
	}
	/**
	...
	public static function css_stripe_media_queries( string $str ): string {
		return (string) \preg_replace( '#@media\s+(?:only\s)?(?:[\s{(]|screen|all)\s?[^{]+{.*}\s*}\s*#isumU', '', $str );
	}
	/**
	...
	public static function emoji_from_country_code( string $country_code_iso_3166_1 ): string {
		...
		$flagOffset              = 0x1f1e6;
		$asciiOffset             = 0x41;
		return ( self::chr( self::ord( $country_code_iso_3166_1[0] ) - $asciiOffset + $flagOffset ) ?? '' ) . ( self::chr( self::ord( $country_code_iso_3166_1[1] ) - $asciiOffset + $flagOffset ) ?? '' );
	}
	/**
	 ...
	 * @return string
	 */
	public static function emoji_decode( string $str, bool $use_reversible_string_mappings = \false ): string {
		if ( self::$EMOJI_KEYS_CACHE === null ) {
			/** @phpstan-ignore-next-line - we need to load the data first */
			self::initEmojiData();
		}
		if ( $use_reversible_string_mappings ) {
			return (string) \str_replace( (array) self::$EMOJI_KEYS_REVERSIBLE_CACHE, (array) self::$EMOJI_VALUES_CACHE, $str );
		}
		return (string) \str_replace( (array) self::$EMOJI_KEYS_CACHE, (array) self::$EMOJI_VALUES_CACHE, $str );
	}
	/**
	 ...
	 * @return string
	 */
	public static function emoji_encode( string $str, bool $use_reversible_string_mappings = \false ): string {
		if ( self::$EMOJI_KEYS_CACHE === null ) {
			/** @phpstan-ignore-next-line - we need to load the data first */
			self::initEmojiData();
		}
		if ( $use_reversible_string_mappings ) {
			return (string) \str_replace( (array) self::$EMOJI_VALUES_CACHE, (array) self::$EMOJI_KEYS_REVERSIBLE_CACHE, $str );
		}
		return (string) \str_replace( (array) self::$EMOJI_VALUES_CACHE, (array) self::$EMOJI_KEYS_CACHE, $str );
	}
	/**
	 ...
	 */
	public static function encode( string $to_encoding, string $str, bool $auto_detect_the_from_encoding = \true, string $from_encoding = '' ): string {
		if ( $str === '' || $to_encoding === '' ) {
			return $str;
		}
		...
		if ( $to_encoding && $from_encoding && $from_encoding === $to_encoding ) {
			return $str;
		}
		...
		if ( $to_encoding === 'HTML-ENTITIES' ) {
			/* @phpstan-ignore-next-line | $str has manybe changed */
			return self::html_encode( $str, \true );
		}
		if ( $auto_detect_the_from_encoding || ! $from_encoding ) {
		...
		if ( $to_encoding === 'UTF-8' && ( $from_encoding === 'WINDOWS-1252' || $from_encoding === 'ISO-8859-1' ) ) {
			/* @phpstan-ignore-next-line | $str has manybe changed */
			return self::to_utf8( $str );
		}
		if ( $to_encoding === 'ISO-8859-1' && ( $from_encoding === 'WINDOWS-1252' || $from_encoding === 'UTF-8' ) ) {
			/* @phpstan-ignore-next-line | $str has manybe changed */
			return self::to_iso8859( $str );
		}
		if ( $to_encoding !== 'UTF-8' && $to_encoding !== 'ISO-8859-1' && $to_encoding !== 'WINDOWS-1252' && self::$SUPPORT['mbstring'] === \false ) {
			/**
			 * @psalm-suppress ImpureFunctionCall - this is only a warning
			 */
		...
		if ( self::$SUPPORT['mbstring'] === \true ) {
			$str_encoded = \mb_convert_encoding( $str, $to_encoding, $from_encoding );
			if ( $str_encoded ) {
	 ...
	 * @return string
	 */
	public static function extract_text( string $str, string $search = '', int $length = null, string $replacer_for_skipped_text = '…', string $encoding = 'UTF-8' ): string {
		if ( $str === '' ) {
			return '';
		}
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
		...
		if ( $search === '' ) {
			if ( $encoding === 'UTF-8' ) {
				...
				$pos = (int) \min( \mb_strpos( $str, ' ', $end ), \mb_strpos( $str, '.', $end ) );
			} else {
				if ( $length > 0 ) {
				...
				$pos = (int) \min( self::strpos( $str, ' ', $end, $encoding ), self::strpos( $str, '.', $end, $encoding ) );
			}
			if ( $pos ) {
				if ( $encoding === 'UTF-8' ) {
		...
		if ( $half_side > 0 ) {
			...
			if ( $half_text !== \false ) {
				if ( $encoding === 'UTF-8' ) {
					$pos_start = (int) \max( \mb_strrpos( $half_text, ' ' ), \mb_strrpos( $half_text, '.' ) );
				} else {
					$pos_start = (int) \max( self::strrpos( $half_text, ' ', 0, $encoding ), self::strrpos( $half_text, '.', 0, $encoding ) );
				}
			}
		}
		if ( $word_position && $half_side > 0 ) {
			...
			if ( $encoding === 'UTF-8' ) {
				$pos_end = (int) \min( \mb_strpos( $str, ' ', $offset ), \mb_strpos( $str, '.', $offset ) ) - $pos_start;
			} else {
				$pos_end = (int) \min( self::strpos( $str, ' ', $offset, $encoding ), self::strpos( $str, '.', $offset, $encoding ) ) - $pos_start;
			}
			...
			if ( $encoding === 'UTF-8' ) {
				$pos_end = (int) \min( \mb_strpos( $str, ' ', $offset ), \mb_strpos( $str, '.', $offset ) );
			} else {
				$pos_end = (int) \min( self::strpos( $str, ' ', $offset, $encoding ), self::strpos( $str, '.', $offset, $encoding ) );
			}
			if ( $pos_end ) {
				if ( $encoding === 'UTF-8' ) {
	 ...
	 */
	public static function file_get_contents( string $filename, bool $use_include_path = \false, $context = null, int $offset = null, int $max_length = null, int $timeout = 10, bool $convert_to_utf8 = \true, string $from_encoding = '' ) {
		// init
		...
		if ( $convert_to_utf8 && ( ! self::is_binary( $data, \true ) || self::is_utf16( $data, \false ) !== \false || self::is_utf32( $data, \false ) !== \false ) ) {
		...
		switch ( \gettype( $var ) ) {
			...
			case 'string':
				...
				if ( ! ASCII::is_ascii( $var ) ) {
					...
					if ( $n && $var[0] >= "\x80" && isset( $n[0], $leading_combining[0] ) && \preg_match( '/^\p{Mn}/u', $var ) ) {
	 ...
	 *                                         </p>
	 */
	public static function filter_input_array( int $type, $definition = null, bool $add_empty = \true ) {
		/**
		 * @psalm-suppress ImpureFunctionCall - we use func_num_args only for args count matching here
		 */
	 ...
	 */
	public static function filter_var( $variable, int $filter = \FILTER_DEFAULT, $options = 0 ) {
		/**
		 * @psalm-suppress ImpureFunctionCall - we use func_num_args only for args count matching here
		 */
		if ( \func_num_args() < 3 ) {
	 ...
	 *                                         </p>
	 */
	public static function filter_var_array( array $data, $definition = 0, bool $add_empty = \true ) {
		/**
		 * @psalm-suppress ImpureFunctionCall - we use func_num_args only for args count matching here
		 */
		if ( \func_num_args() < 2 ) {
	...
	public static function fix_simple_utf8( string $str ): string {
		...
		if ( $BROKEN_UTF8_TO_UTF8_KEYS_CACHE === null ) {
			...
			$BROKEN_UTF8_TO_UTF8_KEYS_CACHE   = \array_keys( self::$BROKEN_UTF8_FIX ?: array() );
	...
	public static function fix_utf8( $str ) {
		...
		$str = (string) $str;
		/* @phpstan-ignore-line | TFixUtf8 is string here */
		$last = '';
		while ( $last !== $str ) {
			$last = $str;
			/**
			 ...
			 */
			$str = self::to_utf8( self::utf8_decode( $str, \true ) );
		}
		/**
		 * @psalm-suppress InvalidReturnStatement
		 */
	 ...
	 * @psalm-pure
	 */
	public static function get_file_type( string $str, array $fallback = array(
		'ext'  => null,
	 ...
	 */
	public static function get_random_string( int $length, string $possible_chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789', string $encoding = 'UTF-8' ): string {
		// init
		$i   = 0;
		$str = '';
		//
	 ...
	 * @return non-empty-string
	 */
	public static function get_unique_string( $extra_entropy = '', bool $use_md5 = \true ): string {
		try {
			$rand_int = \random_int( 0, \mt_getrandmax() );
		} catch ( \Exception $e ) {
			$rand_int = \mt_rand( 0, \mt_getrandmax() );
		}
		$unique_helper = $rand_int . \session_id() . ( $_SERVER['REMOTE_ADDR'] ?? '' ) . ( $_SERVER['SERVER_ADDR'] ?? '' ) . $extra_entropy;
		$unique_string = \uniqid( $unique_helper, \true );
		if ( $use_md5 ) {
	 ...
	 * @phpstan-return (T is non-empty-string ? non-empty-string : string)
	 */
	public static function html_encode( string $str, bool $keep_ascii_chars = \false, string $encoding = 'UTF-8' ): string {
		if ( $str === '' ) {
			return '';
		}
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
		...
		if ( self::$SUPPORT['mbstring'] === \true ) {
			...
			if ( $encoding === 'UTF-8' ) {
				...
				$return = \mb_encode_numericentity( $str, array( $start_code, 0xfffff, 0, 0xfffff ) );
				if ( $return !== null && $return !== \false ) {
					return $return;
				}
			}
			/** @var false|string|null $return - needed for PhpStan (stubs error) */
			$return = \mb_encode_numericentity( $str, array( $start_code, 0xfffff, 0, 0xfffff ), $encoding );
			if ( $return !== null && $return !== \false ) {
				return $return;
			}
		}
	...
	public static function html_escape( string $str, string $encoding = 'UTF-8' ): string {
		return self::htmlspecialchars( $str, \ENT_QUOTES | \ENT_SUBSTITUTE, $encoding );
	}
	/**
	 ...
	 *                </p>
	 */
	public static function htmlentities( string $str, int $flags = \ENT_COMPAT, string $encoding = 'UTF-8', bool $double_encode = \true ): string {
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
			$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
		}
		 ...
		 */
		$str = \str_replace( '\\', '&#92;', $str );
		return self::html_encode( $str, \true, $encoding );
	}
	/**
	 ...
	 * @phpstan-return (T is non-empty-string ? non-empty-string : string)
	 */
	public static function htmlspecialchars( string $str, int $flags = \ENT_COMPAT, string $encoding = 'UTF-8', bool $double_encode = \true ): string {
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
			$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
		}
	...
	public static function is_base64( $str, bool $empty_string_is_valid = \false ): bool {
		if ( ! $empty_string_is_valid && $str === '' ) {
			return \false;
		}
		if ( ! \is_string( $str ) ) {
			return \false;
		}
		$base64String = \base64_decode( $str, \true );
	...
	public static function is_binary( $input, bool $strict = \false ): bool {
		...
		if ( ! $strict ) {
			...
			$test_null_counting = \substr_count( $input, "\x00", 0, $test_length );
	...
	public static function is_binary_file( $file ): bool {
		...
		return self::is_binary( $block, \true );
	}
	/**
	...
	public static function is_html( string $str ): bool {
		...
		// init
		$matches = array();
		$str     = self::emoji_encode( $str );
		// hack for emoji support :/
	...
	public static function is_url( string $url, bool $disallow_localhost = \false ): bool {
		...
		if ( ! self::str_istarts_with_any( $url, array( 'http://', 'https://' ) ) ) {
			return \false;
		}
		// e.g. -> the server itself connect to "https://foo.localhost/phpmyadmin/...
		if ( $disallow_localhost ) {
			if ( self::str_istarts_with_any( $url, array( 'http://localhost', 'https://localhost', 'http://127.0.0.1', 'https://127.0.0.1', 'http://::1', 'https://::1' ) ) ) {
				return \false;
			}
	 ...
	 */
	public static function is_json( string $str, bool $only_array_or_object_results_are_valid = \true ): bool {
		if ( $str === '' ) {
			return \false;
		}
		...
		if ( $only_array_or_object_results_are_valid && ! \is_object( $jsonOrNull ) && ! \is_array( $jsonOrNull ) ) {
			return \false;
		}
	...
	public static function is_serialized( string $str ): bool {
		...
		return $str === 'b:0;' || @\unserialize( $str, array() ) !== \false;
	}
	/**
	 ...
	 */
	public static function is_utf16( $str, bool $check_if_string_is_binary = \true ) {
		// init
		$str       = (string) $str;
		$str_chars = array();
		...
		if ( $check_if_string_is_binary && ! self::is_binary( $str, \true ) ) {
			return \false;
		}
		...
		if ( $test ) {
			...
			if ( $test3 === $test ) {
				$str_chars = self::count_chars( $str, \true, \false );
				foreach ( self::count_chars( $test3 ) as $test3char => &$test3charEmpty ) {
					if ( \in_array( $test3char, $str_chars, \true ) ) {
						++$maybe_utf16le;
					}
				}
	 ...
	 */
	public static function is_utf32( $str, bool $check_if_string_is_binary = \true ) {
		// init
		$str       = (string) $str;
		$str_chars = array();
		// fix for the "binary"-check
	 ...
	 */
	public static function lcfirst( string $str, string $encoding = 'UTF-8', bool $clean_utf8 = \false, string $lang = null, bool $try_to_keep_the_string_length = \false ): string {
		if ( $clean_utf8 ) {
			$str = self::clean( $str );
		}
		...
		if ( $encoding === 'UTF-8' ) {
			...
			if ( $use_mb_functions ) {
				$str_part_one = \mb_strtolower( (string) \mb_substr( $str, 0, 1 ) );
			} else {
				$str_part_one = self::strtolower( (string) \mb_substr( $str, 0, 1 ), $encoding, \false, $lang, $try_to_keep_the_string_length );
			}
		} else {
			$encoding     = self::normalize_encoding( $encoding, 'UTF-8' );
			$str_part_two = (string) self::substr( $str, 1, null, $encoding );
			$str_part_one = self::strtolower( (string) self::substr( $str, 0, 1, $encoding ), $encoding, \false, $lang, $try_to_keep_the_string_length );
		}
	 ...
	 * @return string
	 */
	public static function lcwords( string $str, array $exceptions = array(), string $char_list = '', string $encoding = 'UTF-8', bool $clean_utf8 = \false, string $lang = null, bool $try_to_keep_the_string_length = \false ): string {
		if ( ! $str ) {
			return '';
		}
		$words          = self::str_to_words( $str, $char_list );
		$use_exceptions = $exceptions !== array();
		$words_str      = '';
		foreach ( $words as &$word ) {
			if ( ! $word ) {
				continue;
			}
			if ( ! $use_exceptions || ! \in_array( $word, $exceptions, \true ) ) {
	...
	public static function normalize_encoding( $encoding, $fallback = '' ) {
		...
		if ( $encoding === 'ISO' || $encoding === 'ISO-8859-1' ) {
			return 'ISO-8859-1';
		}
		...
		if ( \in_array( $encoding, self::$ENCODINGS, \true ) ) {
		...
		$equivalences          = array(
			...
			'LATIN9'      => 'ISO-8859-15',
			// Western European (with some extra chars e.g. €)
	 ...
	 */
	public static function normalize_whitespace( string $str, bool $keep_non_breaking_space = \false, bool $keep_bidi_unicode_controls = \false, bool $normalize_control_characters = \false ): string {
	...
	public static function parse_str( string $str, &$result, bool $clean_utf8 = \false ): bool {
		...
		if ( self::$SUPPORT['mbstring'] === \true ) {
			...
			return $return !== \false && $result !== array();
		}
		/**
	 ...
	 * @return list<string>
	 */
	public static function range( $var1, $var2, bool $use_ctype = \true, string $encoding = 'UTF-8', $step = 1 ): array {
		if ( ! $var1 || ! $var2 ) {
			return array();
		}
		if ( $step !== 1 ) {
	 ...
	 * @phpstan-return (T is non-empty-string ? non-empty-string : string)
	 */
	public static function rawurldecode( string $str, bool $multi_decode = \true ): string {
		if ( $str === '' ) {
			return '';
		}
		...
		if ( $multi_decode ) {
			do {
				 ...
				 * @psalm-suppress PossiblyInvalidArgument
				 */
				$str = \rawurldecode( self::html_entity_decode( self::to_utf8( $str ), \ENT_QUOTES | \ENT_HTML5 ) );
			} while ( $str_compare !== $str );
		} else {
			/**
			 * @psalm-suppress PossiblyInvalidArgument
	...
	public static function regex_replace( string $str, string $pattern, string $replacement, string $options = '', string $delimiter = '/' ): string {
		...
		return (string) \preg_replace( $delimiter . $pattern . $delimiter . 'u' . $options, $replacement, $str );
	}
	/**
	 ...
	 */
	public static function remove_invisible_characters( string $str, bool $url_encoded = \false, string $replacement = '', bool $keep_basic_control_characters = \true ): string {
		return ASCII::remove_invisible_characters( $str, $url_encoded, $replacement, $keep_basic_control_characters );
	}
	/**
	...
	public static function remove_left( string $str, string $substring, string $encoding = 'UTF-8' ): string {
		if ( $substring && \strpos( $str, $substring ) === 0 ) {
			if ( $encoding === 'UTF-8' ) {
				return (string) \mb_substr( $str, (int) \mb_strlen( $substring ) );
			}
			$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
			return (string) self::substr( $str, (int) self::strlen( $substring, $encoding ), null, $encoding );
		}
		return $str;
	}
	/**
	...
	public static function remove_right( string $str, string $substring, string $encoding = 'UTF-8' ): string {
		if ( $substring && \substr( $str, -\strlen( $substring ) ) === $substring ) {
			if ( $encoding === 'UTF-8' ) {
				return (string) \mb_substr( $str, 0, (int) \mb_strlen( $str ) - (int) \mb_strlen( $substring ) );
			}
			$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
			return (string) self::substr( $str, 0, (int) self::strlen( $str, $encoding ) - (int) self::strlen( $substring, $encoding ), $encoding );
		}
		return $str;
	}
	/**
	 ...
	 *                <p>A string having a $str without the suffix $substring.</p>
	 */
	public static function remove_iright( string $str, string $substring, string $encoding = 'UTF-8' ): string {
	 ...
	 *                <p>A string without the prefix $substring.</p>
	 */
	public static function remove_ileft( string $str, string $substring, string $encoding = 'UTF-8' ): string {
		if ( $substring && \strpos( self::strtoupper( $str, $encoding ), self::strtoupper( $substring, $encoding ) ) === 0 ) {
			if ( $encoding === 'UTF-8' ) {
				return (string) \mb_substr( $str, (int) \mb_strlen( $substring ) );
			}
			$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
	 ...
	 */
	public static function replace( string $str, string $search, string $replacement, bool $case_sensitive = \true ): string {
		if ( $case_sensitive ) {
	 ...
	 *                <p>A string with replaced parts.</p>
	 */
	public static function replace_all( string $str, array $search, $replacement, bool $case_sensitive = \true ): string {
		if ( $case_sensitive ) {
			return \str_replace( $search, $replacement, $str );
		}
		return self::str_ireplace( $search, $replacement, $str );
	 ...
	 */
	public static function replace_diamond_question_mark( string $str, string $replacement_char = '', bool $process_invalid_utf8_chars = \true ): string {
		if ( $str === '' ) {
			return '';
		}
		...
		return \str_replace( array( '�', '�' ), array( $replacement_char, $replacement_char ), $str );
	}
	/**
	 ...
	 */
	public static function showSupport( bool $useEcho = \true ) {
		// init
		$html  = '';
		$html .= '<pre>';
		foreach ( self::$SUPPORT as $key => &$value ) {
			$html .= $key . ' - ' . \print_r( $value, \true ) . "\n<br>";
		}
		$html .= '</pre>';
		if ( $useEcho ) {
			echo $html;
	 ...
	 * @phpstan-return (T is non-empty-string ? non-empty-string : string)
	 */
	public static function single_chr_html_encode( string $char, bool $keep_ascii_chars = \false, string $encoding = 'UTF-8' ): string {
		if ( $char === '' ) {
			return '';
		}
		if ( $keep_ascii_chars && ASCII::is_ascii( $char ) ) {
			return $char;
		}
	 ...
	 * @return string
	 */
	public static function str_camelize( string $str, string $encoding = 'UTF-8', bool $clean_utf8 = \false, string $lang = null, bool $try_to_keep_the_string_length = \false ): string {
		if ( $clean_utf8 ) {
			$str = self::clean( $str );
		}
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
		...
		$str              = self::lcfirst( \trim( $str ), $encoding, \false, $lang, $try_to_keep_the_string_length );
	...
	public static function str_capitalize_name( string $str ): string {
		return self::str_capitalize_name_helper( self::str_capitalize_name_helper( self::collapse_whitespace( $str ), ' ' ), '-' );
	}
	/**
	 ...
	 */
	public static function str_contains( string $haystack, string $needle, bool $case_sensitive = \true ): bool {
		if ( $case_sensitive ) {
	 ...
	 *              <p>Whether or not $haystack contains $needle.</p>
	 */
	public static function str_contains_all( string $haystack, array $needles, bool $case_sensitive = \true ): bool {
		if ( $haystack === '' || $needles === array() ) {
			return \false;
		}
		foreach ( $needles as &$needle ) {
			if ( $case_sensitive && ( ! $needle || \strpos( $haystack, (string) $needle ) === \false ) ) {
				return \false;
			}
	 ...
	 */
	public static function str_contains_any( string $haystack, array $needles, bool $case_sensitive = \true ): bool {
		if ( $haystack === '' || $needles === array() ) {
			return \false;
		}
		foreach ( $needles as &$needle ) {
	 ...
	 * @phpstan-return (T is non-empty-string ? non-empty-string : string)
	 */
	public static function str_delimit( string $str, string $delimiter, string $encoding = 'UTF-8', bool $clean_utf8 = \false, string $lang = null, bool $try_to_keep_the_string_length = \false ): string {
		if ( self::$SUPPORT['mbstring'] === \true ) {
	...
	public static function str_detect_encoding( $str ) {
		...
		//
		if ( self::is_binary( $str, self::string_has_bom( $str ) ? \false : \true ) ) {
		...
		$encoding_detecting_order = array( 'ISO-8859-1', 'ISO-8859-2', 'ISO-8859-3', 'ISO-8859-4', 'ISO-8859-5', 'ISO-8859-6', 'ISO-8859-7', 'ISO-8859-8', 'ISO-8859-9', 'ISO-8859-10', 'ISO-8859-13', 'ISO-8859-14', 'ISO-8859-15', 'ISO-8859-16', 'WINDOWS-1251', 'WINDOWS-1252', 'WINDOWS-1254', 'CP932', 'CP936', 'CP950', 'CP866', 'CP850', 'CP51932', 'CP50220', 'CP50221', 'CP50222', 'ISO-2022-JP', 'ISO-2022-KR', 'JIS', 'JIS-ms', 'EUC-CN', 'EUC-JP' );
		if ( self::$SUPPORT['mbstring'] === \true ) {
			// info: do not use the symfony polyfill here
			$encoding = \mb_detect_encoding( $str, $encoding_detecting_order, \true );
			if ( $encoding ) {
				return $encoding;
			}
		}
	...
	public static function str_humanize( $str ): string {
		$str = \str_replace( array( '_id', '_' ), array( '', ' ' ), $str );
	...
	public static function str_insert( string $str, string $substring, int $index, string $encoding = 'UTF-8' ): string {
		if ( $encoding === 'UTF-8' ) {
			...
			return (string) \mb_substr( $str, 0, $index ) . $substring . (string) \mb_substr( $str, $index, $len );
		}
		$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
		...
		return (string) self::substr( $str, 0, $index, $encoding ) . $substring . (string) self::substr( $str, $index, $len, $encoding );
	}
	/**
	...
	public static function str_isubstr_after_first_separator( string $str, string $separator, string $encoding = 'UTF-8' ): string {
		...
		if ( $encoding === 'UTF-8' ) {
			return (string) \mb_substr( $str, $offset + (int) \mb_strlen( $separator ) );
		}
		return (string) self::substr( $str, $offset + (int) self::strlen( $separator, $encoding ), null, $encoding );
	}
	/**
	...
	public static function str_isubstr_after_last_separator( string $str, string $separator, string $encoding = 'UTF-8' ): string {
		...
		if ( $encoding === 'UTF-8' ) {
			return (string) \mb_substr( $str, $offset + (int) self::strlen( $separator ) );
		}
		return (string) self::substr( $str, $offset + (int) self::strlen( $separator, $encoding ), null, $encoding );
	}
	/**
	 ...
	 * @return string
	 */
	public static function str_isubstr_first( string $str, string $needle, bool $before_needle = \false, string $encoding = 'UTF-8' ): string {
		if ( $needle === '' || $str === '' ) {
			return '';
		}
		$part = self::stristr( $str, $needle, $before_needle, $encoding );
		if ( $part === \false ) {
			return '';
		}
		return $part;
	 ...
	 * @return string
	 */
	public static function str_isubstr_last( string $str, string $needle, bool $before_needle = \false, string $encoding = 'UTF-8' ): string {
		if ( $needle === '' || $str === '' ) {
			return '';
		}
		$part = self::strrichr( $str, $needle, $before_needle, $encoding );
		if ( $part === \false ) {
			return '';
		}
		return $part;
	 ...
	 * @phpstan-return (T is non-empty-string ? non-empty-string : string)
	 */
	public static function str_limit( string $str, int $length = 100, string $str_add_on = '…', string $encoding = 'UTF-8' ): string {
		if ( $str === '' || $length <= 0 ) {
			return '';
		}
		if ( $encoding === 'UTF-8' ) {
	 ...
	 * @phpstan-return (T is non-empty-string ? non-empty-string : string)
	 */
	public static function str_limit_in_byte( string $str, int $length = 100, string $str_add_on = '...', string $encoding = 'UTF-8' ): string {
		if ( $str === '' || $length <= 0 ) {
			return '';
		}
		$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
	 ...
	 * @phpstan-return (T is non-empty-string ? non-empty-string : string)
	 */
	public static function str_limit_after_word( string $str, int $length = 100, string $str_add_on = '…', string $encoding = 'UTF-8' ): string {
		if ( $str === '' || $length <= 0 ) {
			return '';
		}
		if ( $encoding === 'UTF-8' ) {
	 ...
	 * @return string
	 */
	public static function str_longest_common_prefix( string $str1, string $str2, string $encoding = 'UTF-8' ): string {
		// init
		$longest_common_prefix = '';
		if ( $encoding === 'UTF-8' ) {
			$max_length = (int) \min( \mb_strlen( $str1 ), \mb_strlen( $str2 ) );
			for ( $i = 0; $i < $max_length; ++$i ) {
				$char = \mb_substr( $str1, $i, 1 );
				if ( $char !== \false && $char === \mb_substr( $str2, $i, 1 ) ) {
			...
			$encoding   = self::normalize_encoding( $encoding, 'UTF-8' );
			$max_length = (int) \min( self::strlen( $str1, $encoding ), self::strlen( $str2, $encoding ) );
			for ( $i = 0; $i < $max_length; ++$i ) {
				$char = self::substr( $str1, $i, 1, $encoding );
				if ( $char !== \false && $char === self::substr( $str2, $i, 1, $encoding ) ) {
					$longest_common_prefix .= $char;
				} else {
					break;
				}
	 ...
	 */
	public static function str_longest_common_substring( string $str1, string $str2, string $encoding = 'UTF-8' ): string {
		if ( $str1 === '' || $str2 === '' ) {
			return '';
		}
		...
		$len   = 0;
		$end   = 0;
		$table = \array_fill( 0, $str_length + 1, \array_fill( 0, $other_length + 1, 0 ) );
		if ( $encoding === 'UTF-8' ) {
	 ...
	 * @return string
	 */
	public static function str_longest_common_suffix( string $str1, string $str2, string $encoding = 'UTF-8' ): string {
		if ( $str1 === '' || $str2 === '' ) {
			return '';
		}
		if ( $encoding === 'UTF-8' ) {
			$max_length            = (int) \min( \mb_strlen( $str1, $encoding ), \mb_strlen( $str2, $encoding ) );
	...
	public static function str_pad( string $str, int $pad_length, string $pad_string = ' ', $pad_type = \STR_PAD_RIGHT, string $encoding = 'UTF-8' ): string {
		...
		if ( $pad_type !== (int) $pad_type ) {
			if ( $pad_type === 'left' ) {
				...
				throw new \InvalidArgumentException( 'Pad expects $pad_type to be "STR_PAD_*" or ' . "to be one of 'left', 'right' or 'both'" );
			}
		}
		if ( $encoding === 'UTF-8' ) {
			...
			if ( $pad_length >= $str_length ) {
				switch ( $pad_type ) {
					case \STR_PAD_LEFT:
						...
						$pre       = (string) \mb_substr( \str_repeat( $pad_string, (int) \ceil( $diff / $ps_length ) ), 0, $diff );
						$post      = '';
						break;
					case \STR_PAD_BOTH:
						$diff            = $pad_length - $str_length;
						...
						$pre             = (string) \mb_substr( \str_repeat( $pad_string, $ps_length_left ), 0, $ps_length_left );
						$post            = (string) \mb_substr( \str_repeat( $pad_string, $ps_length_right ), 0, $ps_length_right );
						break;
					case \STR_PAD_RIGHT:
					default:
						$ps_length = (int) \mb_strlen( $pad_string );
						$diff      = $pad_length - $str_length;
						$post      = (string) \mb_substr( \str_repeat( $pad_string, (int) \ceil( $diff / $ps_length ) ), 0, $diff );
						$pre       = '';
				}
				return $pre . $str . $post;
			}
		...
		if ( $pad_length >= $str_length ) {
			switch ( $pad_type ) {
				case \STR_PAD_LEFT:
					...
					$diff      = $pad_length - $str_length;
					$pre       = (string) self::substr( \str_repeat( $pad_string, (int) \ceil( $diff / $ps_length ) ), 0, $diff, $encoding );
					$post      = '';
					break;
				case \STR_PAD_BOTH:
					$diff            = $pad_length - $str_length;
					$ps_length_left  = (int) \floor( $diff / 2 );
					$ps_length_right = (int) \ceil( $diff / 2 );
					$pre             = (string) self::substr( \str_repeat( $pad_string, $ps_length_left ), 0, $ps_length_left, $encoding );
					$post            = (string) self::substr( \str_repeat( $pad_string, $ps_length_right ), 0, $ps_length_right, $encoding );
					break;
				case \STR_PAD_RIGHT:
				default:
					$ps_length = (int) self::strlen( $pad_string, $encoding );
					$diff      = $pad_length - $str_length;
					$post      = (string) self::substr( \str_repeat( $pad_string, (int) \ceil( $diff / $ps_length ) ), 0, $diff, $encoding );
					$pre       = '';
			}
			return $pre . $str . $post;
		}
	 ...
	 */
	public static function str_pad_both( string $str, int $length, string $pad_str = ' ', string $encoding = 'UTF-8' ): string {
		return self::str_pad( $str, $length, $pad_str, \STR_PAD_BOTH, $encoding );
	}
	/**
	 ...
	 */
	public static function str_pad_left( string $str, int $length, string $pad_str = ' ', string $encoding = 'UTF-8' ): string {
		return self::str_pad( $str, $length, $pad_str, \STR_PAD_LEFT, $encoding );
	}
	/**
	 ...
	 */
	public static function str_pad_right( string $str, int $length, string $pad_str = ' ', string $encoding = 'UTF-8' ): string {
		return self::str_pad( $str, $length, $pad_str, \STR_PAD_RIGHT, $encoding );
	}
	/**
	 * Repeat a string.
	 *
		...
		if ( $encoding === 'UTF-8' ) {
			...
			foreach ( $indexes as &$i ) {
				...
				if ( $tmp_sub_str !== \false ) {
					/* @phpstan-ignore-line | old polyfill will return false, or? */
	...
	public static function str_snakeize( string $str, string $encoding = 'UTF-8' ): string {
		...
		$str = \str_replace( '-', '_', self::normalize_whitespace( $str ) );
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
			$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
		}
		$str = (string) \preg_replace_callback(
		...
		return \trim( \trim( $str, '_' ) );
		// trim leading & trailing "_" + whitespace
	}
	/**
	 ...
	 */
	public static function str_split_array( array $input, int $length = 1, bool $clean_utf8 = \false, bool $try_to_use_mb_functions = \true ): array {
		foreach ( $input as &$v ) {
			$v = self::str_split( $v, $length, $clean_utf8, $try_to_use_mb_functions );
		}
	 ...
	 */
	public static function str_split( $str, int $length = 1, bool $clean_utf8 = \false, bool $try_to_use_mb_functions = \true ): array {
		/* @phpstan-ignore-next-line | we do not trust the phpdoc check */
		if ( $length <= 0 ) {
			return array();
		}
		...
		if ( \is_array( $str ) ) {
			...
			return self::str_split_array( $str, $length, $clean_utf8, $try_to_use_mb_functions );
		}
		// init
		$str = (string) $str;
		if ( $str === '' ) {
		...
		if ( $try_to_use_mb_functions && self::$SUPPORT['mbstring'] === \true ) {
			...
			if ( $i_max <= 127 ) {
				...
				$ret = $return_array[0] ?? array();
			}
		} elseif ( self::$SUPPORT['pcre_utf8'] === \true ) {
			$return_array = array();
			\preg_match_all( '/./us', $str, $return_array );
			$ret = $return_array[0] ?? array();
		} else {
			...
			for ( $i = 0; $i < $len; ++$i ) {
				if ( ( $str[ $i ] & "\x80" ) === "\x00" ) {
					$ret[] = $str[ $i ];
				} elseif ( isset( $str[ $i + 1 ] ) && ( $str[ $i ] & "\xe0" ) === "\xc0" ) {
					if ( ( $str[ $i + 1 ] & "\xc0" ) === "\x80" ) {
						$ret[] = $str[ $i ] . $str[ $i + 1 ];
						++$i;
					}
				} elseif ( isset( $str[ $i + 2 ] ) && ( $str[ $i ] & "\xf0" ) === "\xe0" ) {
					if ( ( $str[ $i + 1 ] & "\xc0" ) === "\x80" && ( $str[ $i + 2 ] & "\xc0" ) === "\x80" ) {
						$ret[] = $str[ $i ] . $str[ $i + 1 ] . $str[ $i + 2 ];
						$i    += 2;
					}
				} elseif ( isset( $str[ $i + 3 ] ) && ( $str[ $i ] & "\xf8" ) === "\xf0" ) {
	...
	public static function str_substr_after_first_separator( string $str, string $separator, string $encoding = 'UTF-8' ): string {
		...
		return (string) \mb_substr( $str, $offset + (int) self::strlen( $separator, $encoding ), null, $encoding );
	}
	/**
	 * Gets the substring after the last occurrence of a separator.
	 *
	 ...
	 * @return string
	 */
	public static function str_substr_first( string $str, string $needle, bool $before_needle = \false, string $encoding = 'UTF-8' ): string {
		if ( $str === '' || $needle === '' ) {
			return '';
		}
		if ( $encoding === 'UTF-8' ) {
			if ( $before_needle ) {
				$part = \mb_strstr( $str, $needle, $before_needle );
			} else {
				$part = \mb_strstr( $str, $needle );
			}
		} else {
			$part = self::strstr( $str, $needle, $before_needle, $encoding );
		}
	 ...
	 * @return string
	 */
	public static function str_substr_last( string $str, string $needle, bool $before_needle = \false, string $encoding = 'UTF-8' ): string {
		if ( $str === '' || $needle === '' ) {
			return '';
		}
		if ( $encoding === 'UTF-8' ) {
			if ( $before_needle ) {
				$part = \mb_strrchr( $str, $needle, $before_needle );
			} else {
				$part = \mb_strrchr( $str, $needle );
			}
		} else {
			$part = self::strrchr( $str, $needle, $before_needle, $encoding );
		}
		return $part === \false ? '' : $part;
	}
	/**
	 ...
	 */
	public static function str_titleize( string $str, array $ignore = null, string $encoding = 'UTF-8', bool $clean_utf8 = \false, string $lang = null, bool $try_to_keep_the_string_length = \false, bool $use_trim_first = \true, string $word_define_chars = null ): string {
		if ( $str === '' ) {
			return '';
		}
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
		...
		$str = (string) \preg_replace_callback(
			...
			static function ( array $match ) use ( $try_to_keep_the_string_length, $lang, $ignore, $use_mb_functions, $encoding ): string {
				if ( $ignore !== null && \in_array( $match[0], $ignore, \true ) ) {
					return $match[0];
				}
				if ( $use_mb_functions ) {
					if ( $encoding === 'UTF-8' ) {
					...
					return \mb_strtoupper( \mb_substr( $match[0], 0, 1, $encoding ), $encoding ) . \mb_strtolower( \mb_substr( $match[0], 1, null, $encoding ), $encoding );
				}
				return self::ucfirst( self::strtolower( $match[0], $encoding, \false, $lang, $try_to_keep_the_string_length ), $encoding, \false, $lang, $try_to_keep_the_string_length );
			},
			$str
		);
		return $str;
	 ...
	 */
	public static function str_obfuscate( string $str, float $percent = 0.5, string $obfuscateChar = '*', array $keepChars = array() ): string {
		$obfuscateCharHelper = '☃';
		...
		while ( $charsCounter < $charsMaxChange ) {
			foreach ( $chars as $charKey => $char ) {
				...
				++$charsCounter;
				$charKeyDone[ $charKey ] = \true;
				if ( $charsCounter > $charsMaxChange ) {
					break;
				}
				if ( \in_array( $char, $keepChars, \true ) ) {
					continue;
				}
	 ...
	 *                <p>The titleized string.</p>
	 */
	public static function str_titleize_for_humans( string $str, array $ignore = array(), string $encoding = 'UTF-8' ): string {
		if ( $str === '' ) {
			return '';
		}
		$small_words = array( '(?<!q&)a', 'an', 'and', 'as', 'at(?!&t)', 'but', 'by', 'en', 'for', 'if', 'in', 'of', 'on', 'or', 'the', 'to', 'v[.]?', 'via', 'vs[.]?' );
		if ( $ignore !== array() ) {
	...
	public static function str_to_lines( string $str, bool $remove_empty_values = \false, int $remove_short_values = null ): array {
		if ( $str === '' ) {
			return $remove_empty_values ? array() : array( '' );
		}
		if ( self::$SUPPORT['mbstring'] === \true ) {
			$return = \mb_split( "[\r\n]{1,2}", $str );
		} else {
		...
		if ( $remove_short_values === null && ! $remove_empty_values ) {
			return $return;
		}
		return self::reduce_string_array( $return, $remove_empty_values, $remove_short_values );
	}
	/**
	 ...
	 */
	public static function str_to_words( string $str, string $char_list = '', bool $remove_empty_values = \false, int $remove_short_values = null ): array {
		if ( $str === '' ) {
			return $remove_empty_values ? array() : array( '' );
		}
		...
		$tmp_return = self::reduce_string_array( $return, $remove_empty_values, $remove_short_values );
	 ...
	 */
	public static function str_truncate( string $str, int $length, string $substring = '', string $encoding = 'UTF-8' ): string {
		if ( $str === '' ) {
			return '';
		}
		if ( $encoding === 'UTF-8' ) {
	 ...
	 *                <p>A string after truncating.</p>
	 */
	public static function str_truncate_safe( string $str, int $length, string $substring = '', string $encoding = 'UTF-8', bool $ignore_do_not_split_words_for_one_word = \false ): string {
		if ( $str === '' || $length <= 0 ) {
			return $substring;
		}
		if ( $encoding === 'UTF-8' ) {
			...
			if ( $space_position !== $length ) {
				...
				if ( $last_position !== \false || $space_position !== \false && ! $ignore_do_not_split_words_for_one_word ) {
	 ...
	 */
	public static function str_upper_camelize( string $str, string $encoding = 'UTF-8', bool $clean_utf8 = \false, string $lang = null, bool $try_to_keep_the_string_length = \false ): string {
	...
	public static function strcasecmp( string $str1, string $str2, string $encoding = 'UTF-8' ): int {
		return self::strcmp( self::strtocasefold( $str1, \true, \false, $encoding, null, \false ), self::strtocasefold( $str2, \true, \false, $encoding, null, \false ) );
	}
	/**
	...
	public static function strcmp( string $str1, string $str2 ): int {
		...
		return \strcmp( \Normalizer::normalize( $str1, \Normalizer::NFD ), \Normalizer::normalize( $str2, \Normalizer::NFD ) );
	}
	/**
	 ...
	 */
	public static function strcspn( string $str, string $char_list, int $offset = 0, int $length = null, string $encoding = 'UTF-8' ): int {
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
			$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
		}
		if ( $char_list === '' ) {
	 ...
	 */
	public static function stripos( string $haystack, string $needle, int $offset = 0, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		if ( $haystack === '' ) {
		...
		$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
		if ( $encoding === 'UTF-8' && $offset >= 0 && self::$SUPPORT['intl'] === \true ) {
		...
		// fallback via vanilla php
		//
		$haystack = self::strtocasefold( $haystack, \true, \false, $encoding, null, \false );
		$needle   = self::strtocasefold( $needle, \true, \false, $encoding, null, \false );
	 ...
	 */
	public static function stristr( string $haystack, string $needle, bool $before_needle = \false, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		if ( $haystack === '' ) {
			if ( \PHP_VERSION_ID >= 80000 && $needle === '' ) {
				return '';
			}
		...
		$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
		if ( $encoding !== 'UTF-8' && self::$SUPPORT['mbstring'] === \false ) {
			/**
			 * @psalm-suppress ImpureFunctionCall - this is only a warning
			 */
			\trigger_error( 'UTF8::stristr() without mbstring cannot handle "' . $encoding . '" encoding', \E_USER_WARNING );
		}
		if ( $encoding === 'UTF-8' && self::$SUPPORT['intl'] === \true ) {
	 ...
	 * @phpstan-return false|0|positive-int
	 */
	public static function strlen( string $str, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		if ( $str === '' ) {
			return 0;
		}
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
		...
		if ( $encoding !== 'UTF-8' && self::$SUPPORT['mbstring'] === \false && self::$SUPPORT['iconv'] === \false ) {
			/**
			 * @psalm-suppress ImpureFunctionCall - this is only a warning
			 */
	...
	public static function strnatcasecmp( string $str1, string $str2, string $encoding = 'UTF-8' ): int {
		return self::strnatcmp( self::strtocasefold( $str1, \true, \false, $encoding, null, \false ), self::strtocasefold( $str2, \true, \false, $encoding, null, \false ) );
	}
	/**
	...
	public static function strnatcmp( string $str1, string $str2 ): int {
		...
		return \strnatcmp( (string) self::strtonatfold( $str1 ), (string) self::strtonatfold( $str2 ) );
	}
	/**
	...
	public static function strncasecmp( string $str1, string $str2, int $len, string $encoding = 'UTF-8' ): int {
		return self::strncmp( self::strtocasefold( $str1, \true, \false, $encoding, null, \false ), self::strtocasefold( $str2, \true, \false, $encoding, null, \false ), $len );
	}
	/**
	 ...
	 *             <strong>0</strong> if they are equal
	 */
	public static function strncmp( string $str1, string $str2, int $len, string $encoding = 'UTF-8' ): int {
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
			$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
		}
		if ( $encoding === 'UTF-8' ) {
	 ...
	 * @phpstan-return false|0|positive-int
	 */
	public static function strpos( string $haystack, $needle, int $offset = 0, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		if ( $haystack === '' ) {
			if ( \PHP_VERSION_ID >= 80000 ) {
				if ( $needle === '' ) {
					return 0;
		...
		if ( $encoding !== 'UTF-8' && self::$SUPPORT['iconv'] === \false && self::$SUPPORT['mbstring'] === \false ) {
			/**
			 * @psalm-suppress ImpureFunctionCall - this is only a warning
			 */
		...
		// fallback via iconv
		//
		if ( $offset >= 0 && self::$SUPPORT['iconv'] === \true ) {
	...
	public static function strpos_in_byte( string $haystack, string $needle, int $offset = 0 ) {
		...
		if ( self::$SUPPORT['mbstring_func_overload'] === \true ) {
			// "mb_" is available if overload is used, so use it ...
			return \mb_strpos( $haystack, $needle, $offset, 'CP850' );
			// 8-BIT
		}
		return \strpos( $haystack, $needle, $offset );
	}
	...
	public static function stripos_in_byte( string $haystack, string $needle, int $offset = 0 ) {
		...
		if ( self::$SUPPORT['mbstring_func_overload'] === \true ) {
			// "mb_" is available if overload is used, so use it ...
			return \mb_stripos( $haystack, $needle, $offset, 'CP850' );
			// 8-BIT
		}
		return \stripos( $haystack, $needle, $offset );
	}
	 ...
	 */
	public static function strrchr( string $haystack, string $needle, bool $before_needle = \false, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		if ( $haystack === '' || $needle === '' ) {
			return \false;
		}
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
		...
		// fallback for binary || ascii only
		//
		if ( ! $before_needle && ( $encoding === 'CP850' || $encoding === 'ASCII' ) ) {
	...
	public static function strrev( string $str, string $encoding = 'UTF-8' ): string {
		...
		// init
		$reversed = '';
		$str      = self::emoji_encode( $str, \true );
		if ( $encoding === 'UTF-8' ) {
			if ( self::$SUPPORT['intl'] === \true ) {
		...
		return self::emoji_decode( $reversed, \true );
	}
	/**
	 ...
	 */
	public static function strrichr( string $haystack, string $needle, bool $before_needle = \false, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		if ( $haystack === '' || $needle === '' ) {
			return \false;
		}
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
	 ...
	 */
	public static function strripos( string $haystack, $needle, int $offset = 0, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		if ( $haystack === '' ) {
			if ( \PHP_VERSION_ID >= 80000 ) {
				if ( $needle === '' ) {
					return 0;
		...
		// fallback via vanilla php
		//
		$haystack = self::strtocasefold( $haystack, \true, \false, $encoding );
		$needle   = self::strtocasefold( $needle, \true, \false, $encoding );
	...
	public static function strripos_in_byte( string $haystack, string $needle, int $offset = 0 ) {
		...
		if ( self::$SUPPORT['mbstring_func_overload'] === \true ) {
			// "mb_" is available if overload is used, so use it ...
			return \mb_strripos( $haystack, $needle, $offset, 'CP850' );
			// 8-BIT
		}
		return \strripos( $haystack, $needle, $offset );
	}
	 ...
	 *                   string.<br>If needle is not found, it returns false.</p>
	 */
	public static function strrpos( string $haystack, $needle, int $offset = 0, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		if ( $haystack === '' ) {
			if ( \PHP_VERSION_ID >= 80000 ) {
				if ( $needle === '' ) {
					return 0;
		...
		// fallback via intl
		//
		if ( $offset >= 0 && $encoding === 'UTF-8' && self::$SUPPORT['intl'] === \true ) {
	...
	public static function strrpos_in_byte( string $haystack, string $needle, int $offset = 0 ) {
		...
		if ( self::$SUPPORT['mbstring_func_overload'] === \true ) {
			// "mb_" is available if overload is used, so use it ...
			return \mb_strrpos( $haystack, $needle, $offset, 'CP850' );
			// 8-BIT
		}
		return \strrpos( $haystack, $needle, $offset );
	}
	 ...
	 * @return false|int
	 */
	public static function strspn( string $str, string $mask, int $offset = 0, int $length = null, string $encoding = 'UTF-8' ) {
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
			$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
		}
		if ( $offset || $length !== null ) {
	 ...
	 *                      <p>A sub-string,<br>or <strong>false</strong> if needle is not found.</p>
	 */
	public static function strstr( string $haystack, string $needle, bool $before_needle = \false, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		if ( $haystack === '' ) {
			if ( \PHP_VERSION_ID >= 80000 && $needle === '' ) {
				return '';
			}
	...
	public static function strstr_in_byte( string $haystack, string $needle, bool $before_needle = \false ) {
		...
		if ( self::$SUPPORT['mbstring_func_overload'] === \true ) {
			// "mb_" is available if overload is used, so use it ...
			return \mb_strstr( $haystack, $needle, $before_needle, 'CP850' );
			// 8-BIT
		}
		return \strstr( $haystack, $needle, $before_needle );
	}
	 ...
	 * @return string
	 */
	public static function strtocasefold( string $str, bool $full = \true, bool $clean_utf8 = \false, string $encoding = 'UTF-8', string $lang = null, bool $lower = \true ): string {
		if ( $str === '' ) {
			return '';
		}
		if ( $clean_utf8 ) {
	 ...
	 */
	public static function strtolower( $str, string $encoding = 'UTF-8', bool $clean_utf8 = \false, string $lang = null, bool $try_to_keep_the_string_length = \false ): string {
		// init
		$str = (string) $str;
		if ( $str === '' ) {
			return '';
		...
		if ( $try_to_keep_the_string_length ) {
			$str = self::fixStrCaseHelper( $str, \true );
		}
		if ( $lang === null && $encoding === 'UTF-8' ) {
			return \mb_strtolower( $str );
		}
		...
		if ( $lang !== null ) {
			if ( self::$SUPPORT['intl'] === \true ) {
				...
				if ( ! \in_array( $language_code, self::$INTL_TRANSLITERATOR_LIST, \true ) ) {
					/**
					 * @psalm-suppress ImpureFunctionCall - this is only a warning
					 */
					\trigger_error( 'UTF8::strtolower() cannot handle special language: ' . $lang . ' | supported: ' . \print_r( self::$INTL_TRANSLITERATOR_LIST, \true ), \E_USER_WARNING );
	 ...
	 */
	public static function strtoupper( $str, string $encoding = 'UTF-8', bool $clean_utf8 = \false, string $lang = null, bool $try_to_keep_the_string_length = \false ): string {
		// init
		$str = (string) $str;
		if ( $str === '' ) {
			return '';
	...
	public static function strtr( string $str, $from, $to = '' ): string {
		...
		if ( $to !== '' ) {
			...
			if ( $from === \false ) {
				throw new \InvalidArgumentException( 'The number of elements for each array isn\'t equal or the arrays are empty: (from: ' . \print_r( $from, \true ) . ' | to: ' . \print_r( $to, \true ) . ')' );
			}
		}
		if ( \is_string( $from ) ) {
	 ...
	 * @phpstan-return 0|positive-int
	 */
	public static function strwidth( string $str, string $encoding = 'UTF-8', bool $clean_utf8 = \false ): int {
		if ( $str === '' ) {
			return 0;
		}
		if ( $encoding !== 'UTF-8' && $encoding !== 'CP850' ) {
	 ...
	 *                      characters long, <b>FALSE</b> will be returned.
	 */
	public static function substr( string $str, int $offset = 0, int $length = null, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		// empty string
		if ( $str === '' || $length === 0 ) {
			return '';
		}
		...
		// fallback via mbstring
		//
		if ( self::$SUPPORT['mbstring'] === \true && $encoding === 'UTF-8' ) {
			if ( $length === null ) {
		...
		// fallback via iconv
		//
		if ( $length >= 0 && self::$SUPPORT['iconv'] === \true ) {
	 ...
	 *             <strong>0</strong> if they are equal
	 */
	public static function substr_compare( string $str1, string $str2, int $offset = 0, int $length = null, bool $case_insensitivity = \false, string $encoding = 'UTF-8' ): int {
	 ...
	 */
	public static function substr_count( string $haystack, string $needle, int $offset = 0, int $length = null, string $encoding = 'UTF-8', bool $clean_utf8 = \false ) {
		if ( $needle === '' ) {
			return \false;
		}
	...
	public static function substr_count_in_byte( string $haystack, string $needle, int $offset = 0, int $length = null ) {
		...
		if ( ( $offset || $length !== null ) && self::$SUPPORT['mbstring_func_overload'] === \true ) {
			if ( $length === null ) {
		...
		if ( self::$SUPPORT['mbstring_func_overload'] === \true ) {
			// "mb_" is available if overload is used, so use it ...
			return \mb_substr_count( $haystack, $needle, 'CP850' );
			// 8-BIT
		}
		if ( $length === null ) {
	 ...
	 * @phpstan-return 0|positive-int
	 */
	public static function substr_count_simple( string $str, string $substring, bool $case_sensitive = \true, string $encoding = 'UTF-8' ): int {
		if ( $str === '' || $substring === '' ) {
			return 0;
		}
		if ( $encoding === 'UTF-8' ) {
			...
			return (int) \mb_substr_count( \mb_strtoupper( $str ), \mb_strtoupper( $substring ) );
		}
		$encoding = self::normalize_encoding( $encoding, 'UTF-8' );
		if ( $case_sensitive ) {
			return (int) \mb_substr_count( $str, $substring, $encoding );
		}
		return (int) \mb_substr_count( self::strtocasefold( $str, \true, \false, $encoding, null, \false ), self::strtocasefold( $substring, \true, \false, $encoding, null, \false ), $encoding );
	}
	/**
	...
	public static function substr_in_byte( string $str, int $offset = 0, int $length = null ) {
		...
		if ( self::$SUPPORT['mbstring_func_overload'] === \true ) {
			// "mb_" is available if overload is used, so use it ...
			return \mb_substr( $str, $offset, $length, 'CP850' );
			// 8-BIT
		}
	...
	public static function substr_replace( $str, $replacement, $offset, $length = null, string $encoding = 'UTF-8' ) {
		if ( \is_array( $str ) ) {
			...
			return \array_map( array( self::class, 'substr_replace' ), $str, $replacement, $offset, $length );
		}
		if ( \is_array( $replacement ) ) {
		...
		if ( self::$SUPPORT['mbstring'] === \true ) {
			...
			return (string) \mb_substr( $str, 0, $offset, $encoding ) . $replacement . (string) \mb_substr( $str, $offset + $length, $string_length - $offset - $length, $encoding );
		}
		//
		// fallback for ascii only
		//
	 ...
	 *                <p>Return the sub-string.</p>
	 */
	public static function substr_right( string $haystack, string $needle, string $encoding = 'UTF-8' ): string {
		if ( $haystack === '' ) {
			return '';
		}
		if ( $needle === '' ) {
		...
		if ( $encoding === 'UTF-8' && \substr( $haystack, -\strlen( $needle ) ) === $needle ) {
			return (string) \mb_substr( $haystack, 0, (int) \mb_strlen( $haystack ) - (int) \mb_strlen( $needle ) );
		}
		if ( \substr( $haystack, -\strlen( $needle ) ) === $needle ) {
			return (string) self::substr( $haystack, 0, (int) self::strlen( $haystack, $encoding ) - (int) self::strlen( $needle, $encoding ), $encoding );
		}
		return $haystack;
	}
	/**
	 ...
	 */
	public static function titlecase( string $str, string $encoding = 'UTF-8', bool $clean_utf8 = \false, string $lang = null, bool $try_to_keep_the_string_length = \false ): string {
		if ( $clean_utf8 ) {
			// "mb_strpos()" and "iconv_strpos()" returns wrong position,
			// if invalid characters are found in $haystack before $needle
			$str = self::clean( $str );
		}
		if ( $lang === null && ! $try_to_keep_the_string_length ) {
			if ( $encoding === 'UTF-8' ) {
		...
		return self::str_titleize( $str, null, $encoding, \false, $lang, $try_to_keep_the_string_length, \false );
	}
	/**
	 ...
	 * @return string
	 */
	public static function to_filename( string $str, bool $use_transliterate = \false, string $fallback_char = '-' ): string {
		return ASCII::to_filename( $str, $use_transliterate, $fallback_char );
	}
	/**
		...
		$buf = \preg_replace_callback(
			...
			static function ( array $matches ): string {
				if ( isset( $matches[3] ) ) {
					...
					$cp = ( (int) \hexdec( $matches[1] ) << 10 ) + (int) \hexdec( $matches[2] ) + 0x10000 - ( 0xd800 << 10 ) - 0xdc00;
				}
	...
	public static function to_string( $input ) {
		...
		if ( $input_type === 'string' || $input_type === 'integer' || $input_type === 'float' || $input_type === 'double' ) {
	 ...
	 */
	public static function ucfirst( string $str, string $encoding = 'UTF-8', bool $clean_utf8 = \false, string $lang = null, bool $try_to_keep_the_string_length = \false ): string {
		if ( $str === '' ) {
			return '';
		}
		if ( $clean_utf8 ) {
		...
		if ( $encoding === 'UTF-8' ) {
			...
			if ( $use_mb_functions ) {
				...
				$str_part_one = self::strtoupper( (string) \mb_substr( $str, 0, 1 ), $encoding, \false, $lang, $try_to_keep_the_string_length );
			}
		} else {
			$encoding     = self::normalize_encoding( $encoding, 'UTF-8' );
			$str_part_two = (string) self::substr( $str, 1, null, $encoding );
			if ( $use_mb_functions ) {
				$str_part_one = \mb_strtoupper( (string) \mb_substr( $str, 0, 1, $encoding ), $encoding );
			} else {
				$str_part_one = self::strtoupper( (string) self::substr( $str, 0, 1, $encoding ), $encoding, \false, $lang, $try_to_keep_the_string_length );
			}
		}
		return $str_part_one . $str_part_two;
	}
	 ...
	 * @return string
	 */
	public static function ucwords( string $str, array $exceptions = array(), string $char_list = '', string $encoding = 'UTF-8', bool $clean_utf8 = \false ): string {
		if ( ! $str ) {
			return '';
		}
		...
		if ( $use_php_default_functions && ASCII::is_ascii( $str ) ) {
			return \ucwords( $str );
		}
		$words          = self::str_to_words( $str, $char_list );
		$use_exceptions = $exceptions !== array();
	 ...
	 * @phpstan-return (T is non-empty-string ? non-empty-string : string)
	 */
	public static function urldecode( string $str, bool $multi_decode = \true ): string {
		if ( $str === '' ) {
			return '';
		}
		$str = self::urldecode_unicode_helper( $str );
		if ( $multi_decode ) {
			do {
				 ...
				 * @psalm-suppress PossiblyInvalidArgument
				 */
				$str = \urldecode( self::html_entity_decode( self::to_utf8( $str ), \ENT_QUOTES | \ENT_HTML5 ) );
			} while ( $str_compare !== $str );
		} else {
			/**
			 * @psalm-suppress PossiblyInvalidArgument
	...
	public static function utf8_decode( string $str, bool $keep_utf8_chars = \false ): string {
		...
		for ( $i = 0, $j = 0; $i < $len; ++$i, ++$j ) {
			switch ( $str[ $i ] & "\xf0" ) {
				case "\xc0":
				case "\xd0":
					$c         = self::$ORD[ $str[ $i ] & "\x1f" ] << 6 | self::$ORD[ $str[ ++$i ] & '?' ];
		...
		if ( $keep_utf8_chars && (int) self::strlen( $return ) >= (int) self::strlen( $str_backup ) ) {
			return $str_backup;
		}
		return $return;
	}
	 ...
	 * @return string
	 */
	public static function words_limit( string $str, int $limit = 100, string $str_add_on = '…' ): string {
		if ( $str === '' || $limit <= 0 ) {
			return '';
		}
		\preg_match( '/^\s*+(?:[^\s]++\s*+){1,' . $limit . '}/u', $str, $matches );
		if ( ! isset( $matches[0] ) || \mb_strlen( $str ) === (int) \mb_strlen( $matches[0] ) ) {
			return $str;
		}
	 ...
	 * @return string
	 */
	public static function wordwrap_per_line( string $str, int $width = 75, string $break = "\n", bool $cut = \false, bool $add_final_break = \true, string $delimiter = null ): string {
		if ( $delimiter === null ) {
			$strings = \preg_split( '/\r\n|\r|\n/', $str );
		} else {
			$strings = \explode( $delimiter, $str );
		}
		$string_helper_array = array();
		if ( $strings !== \false ) {
	...
	private static function is_utf8_string( string $str, bool $strict = \false ) {
		...
		if ( $strict ) {
			$is_binary = self::is_binary( $str, \true );
		...
		for ( $i = 0; $i < $len; ++$i ) {
			...
			if ( $mState === 0 ) {
				...
				if ( --$mState === 0 ) {
					...
					if ( $mBytes === 2 && $mUcs4 < 0x80 || $mBytes === 3 && $mUcs4 < 0x800 || $mBytes === 4 && $mUcs4 < 0x10000 || $mBytes > 4 || ( $mUcs4 & 0xfffff800 ) === 0xd800 || $mUcs4 > 0x10ffff ) {
						return \false;
					}
	 ...
	 * @return string
	 */
	private static function fixStrCaseHelper( string $str, bool $use_lowercase = \false, bool $use_full_case_fold = \false ) {
	 ...
	 * @return list<string>
	 */
	private static function reduce_string_array( array $strings, bool $remove_empty_values, int $remove_short_values = null ) {
		// init
		$return = array();
		foreach ( $strings as &$str ) {
			if ( $remove_short_values !== null && \mb_strlen( $str ) <= $remove_short_values ) {
				continue;
			}
			if ( $remove_empty_values && \trim( $str ) === '' ) {
				continue;
			}
			$return[] = $str;
		}
	...
	private static function str_capitalize_name_helper( string $names, string $delimiter, string $encoding = 'UTF-8' ) {
		...
		$special_cases = array(
			'names'    => array( 'ab', 'af', 'al', 'and', 'ap', 'bint', 'binte', 'da', 'de', 'del', 'den', 'der', 'di', 'dit', 'ibn', 'la', 'mac', 'nic', 'of', 'ter', 'the', 'und', 'van', 'von', 'y', 'zu' ),
			'prefixes' => array( 'al-', "d'", 'ff', "l'", 'mac', 'mc', 'nic' ),
		);
		foreach ( $name_helper_array as &$name ) {
			if ( \in_array( $name, $special_cases['names'], \true ) ) {
				continue;
			}
			$continue = \false;
			if ( $delimiter === '-' ) {
	...
	private static function to_utf8_convert_helper( $input ) {
		...
		if ( isset( self::$WIN1252_TO_UTF8[ $ordC1 ] ) ) {
			// found in Windows-1252 special cases
			$buf .= self::$WIN1252_TO_UTF8[ $ordC1 ];
		} else {
			$cc1  = self::$CHR[ $ordC1 / 64 ] | "\xc0";
			$cc2  = (string) $input & '?' | "\x80";
			$buf .= $cc1 . $cc2;
		}
		return $buf;
	}
