...
class JobsitesAccordion {
  ...
  initJobsitesFeatures(context = document) {
    ...
    jobsitesBlocks.forEach(block => {
      ...
      // Helper: filter to show only one letter, or show all
      let activeLetter = null;
      function filterByLetter(letter) {
        // If same letter clicked again, show all
        if (activeLetter === letter) {
          activeLetter = null;
          letterSections.forEach(s => s.classList.remove('is-hidden'));
          letterButtons.forEach(btn => btn.classList.remove('active'));
          if (skipSelect) skipSelect.value = '';
          return;
        }
        ...
        letterSections.forEach(s => {
          s.classList.toggle('is-hidden', s.dataset.letter !== letter);
        });
        letterButtons.forEach(btn => {
          btn.classList.toggle('active', btn.dataset.letter === letter);
        });

        // Scroll container to top when filtering
      ...
      letterButtons.forEach(button => {
        button.addEventListener('click', () => {
          filterByLetter(button.dataset.letter);
        });
      });
      ...
      if (skipSelect) {
        skipSelect.addEventListener('change', e => {
          ...
          if (letter) {
            ...
            // "Select a letter..." resets to show all
            activeLetter = null;
            letterSections.forEach(s => s.classList.remove('is-hidden'));
            letterButtons.forEach(btn => btn.classList.remove('active'));
          }
