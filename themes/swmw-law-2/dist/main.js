...
class JobsitesAccordion {
  ...
  initJobsitesFeatures(context = document) {
    ...
    jobsitesBlocks.forEach(block => {
      ...
      // Helper: scroll the page so the target letter heading is visible
      function jumpToLetter(letter) {
        const target = block.querySelector(`#letter-${letter}`);
        if (!target) return;

        // First scroll the page to bring the scroll container into view
        if (scrollContainer) {
        ...
        // Highlight the target with a brief flash
