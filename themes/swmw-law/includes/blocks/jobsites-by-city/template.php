...
<div <?php echo wp_kses_post(get_block_wrapper_attributes(['class' => 'jobsites-by-city-block'])); ?>>
    <!-- Search Bar (searches both city names and company/jobsite names) -->
    <div class="jobsites-search">
        <input type="text" placeholder="Search cities or company names..." class="jobsites-search-input" aria-label="Search cities or company names" />
        <button class="jobsites-search-clear" aria-label="Clear search" hidden>&times;</button>
    </div>

    <!-- A-Z Jump Nav (Desktop) -->
        ...
        <div class="filter-letters">
            <?php foreach (range('A', 'Z') as $letter): ?>
                ...
                <button
                    ...
                    aria-label="<?php echo esc_attr(sprintf('Jump to cities starting with %s', $letter)); ?>"
                >
                    <?php echo esc_html($letter); ?>
                </button>
            <?php endforeach; ?>
        </div>
        <button type="button" class="jobsites-reset-letter" aria-label="Show all letters" title="Show all">×</button>
    </div>

    <!-- Skip to Dropdown (Mobile) -->
    <div class="jobsites-skip-to">
        <label for="skip-to-<?php echo esc_attr($block_unique_id); ?>" class="skip-to-label">Jump to:</label>
    ...
    <!-- Scroll Container -->
    <div class="jobsites-directory-scroll">
        <div class="jobsites-directory" id="<?php echo esc_attr($block_unique_id); ?>">
            <?php foreach ($letters_with_cities as $letter): ?>
                <?php $cities = $cities_by_letter[$letter]; ?>
                <div class="letter-section" id="letter-<?php echo esc_attr($letter); ?>" data-letter="<?php echo esc_attr($letter); ?>">
                    <h3 class="letter-heading"><?php echo esc_html($letter); ?></h3>
                    <div class="letter-cities">
                        <?php foreach ($cities as $city): ?>
                            <div class="city-group" data-city="<?php echo esc_attr($city['title']); ?>">
                                <h4 class="city-name"><?php echo esc_html($city['title']); ?>:</h4>
                                <?php if (have_rows('jobsites', $city['id'])): ?>
    ...
    <p class="jobsites-no-results" hidden>No cities match your search.</p>
</div>
